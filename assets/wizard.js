...
const STEPS = [
 ...
 /* ===== MODIFICA: divulgazione DESIGN resta qui (come prima) ===== */
 ...
 /* ===== MODIFICA: divulgazione BREVE-TO dedicata (una scelta “Sì” + stop) ===== */
 {id:'public_inv',question:'Hai già divulgato/pubblicato l’invenzione (o dettagli tecnici sufficienti) prima del deposito?',subtitle:'Per divulgazione intendiamo: post, pitch pubblico, demo, fiera, vendita/offerta, paper, video, invio a terzi senza riservatezza.',type:'single',
  showIf:function(a){return a.what==='invention';},
  options:[
   {value:'no',label:'No, è ancora riservata',desc:'Non è stata resa disponibile al pubblico.'},
   {value:'yes_pub',label:'Sì, ho già divulgato/pubblicato',desc:'È stata resa pubblica (anche online o a una fiera) prima di depositare.'},
   {value:'unsure_pub',label:'Non sono sicuro',desc:'Ricostruiremo cosa è stato divulgato e quando.'}
  ]},
...
\u2022 conserva prove di date e versioni (file, email, repository, screenshots).`},
 ...
 public_inv:{title:'Brevetto e divulgazione: perché è “pericolosa”',
 content:`Per un brevetto conta la <b>novità</b>. In UE/Italia una divulgazione pubblica prima del deposito <b>di norma distrugge la novità</b> e impedisce il brevetto (salvo eccezioni molto rare e ristrette).

Divulgazione = rendere disponibile al pubblico: post online, pitch registrato, demo aperta, fiera, offerta/vendita, video, paper, invio a terzi senza riservatezza.

Se hai già divulgato:
\u2022 non divulgare altro
\u2022 ricostruisci <b>cosa</b> e <b>quando</b> è stato reso pubblico
\u2022 valuta alternative (miglioramenti/nuove varianti, segreto su parti non divulgate, contratti/NDA) e una consulenza rapida.`},
...
function getRec(a){
  ...
  if(isActionPath){
    ...
    if(w==='invention'){
      ...
      if(pubInv==='yes_pub'){
        rec.title = "Brevetto: divulgazione già avvenuta (stop)";
        rec.text = "Hai indicato che l'invenzione (o dettagli tecnici sufficienti) è già stata divulgata/pubblicata prima del deposito. In UE/Italia una divulgazione pubblica precedente al deposito <b>di norma distrugge la novità e impedisce il brevetto</b> (salvo eccezioni molto rare e ristrette).";
        rec.next = [
          "Non divulgare altro: ogni nuovo dettaglio può peggiorare la situazione.",
          "Ricostruisci con precisione <b>cosa</b> è stato divulgato e <b>quando</b> (URL, post, video, fiere, offerte, vendite, email senza NDA).",
          "Valuta rapidamente alternative: <b>miglioramenti/varianti</b> non divulgati (eventuale nuovo deposito), <b>segreto</b> su parti rimaste riservate, e <b>contratti/NDA</b> per collaborazioni e sfruttamento.",
          "Se ti serve capire se rientri in una rara eccezione o se ci sono margini, serve una valutazione mirata (consulenza)."
        ];
      } else {
...
/* ===== MODIFICA: stop brevetto (salto a summary) ===== */
function goNext(){
  if(!(canNext() && step<totalSteps()-1)) return;
  var c=cur();
  if(c && c.id==='public_inv' && answers.public_inv==='yes_pub'){
    step = totalSteps()-1; // vai direttamente al riepilogo
    render();
    return;
  }
  step++;
...
function getLearn(a){
  ...
  /* ===== MODIFICA: per invenzione usa public_inv ===== */
  var publicState = (what==='invention') ? (a.public_inv||'') : (a.public||'');
...
function renderCore(){
 ...
 if(c.type==='single'||c.type==='multi'){
   ...
   /* ===== MODIFICA: warning inline sotto la domanda (design / brevetto) ===== */
   if(c.id==='public'){
     if(answers.public==='less_12'){
       h+='<p class="wz-sub" style="margin-top:12px;border:1px solid rgba(198,40,40,.25);background:rgba(198,40,40,.06);padding:10px;border-radius:10px">'
         +IC.alert+' <b>Attenzione:</b> dopo la divulgazione il margine è limitato. Per il design UE spesso esiste un periodo di grazia, ma conviene muoversi presto e ordinare le prove (date/URL/foto/versioni).'
         +'</p>';
     } else if(answers.public==='more_12'){
       h+='<p class="wz-sub" style="margin-top:12px;border:1px solid rgba(198,40,40,.25);background:rgba(198,40,40,.06);padding:10px;border-radius:10px">'
         +IC.alert+' <b>Attenzione:</b> se la divulgazione è > 12 mesi, una registrazione del design UE può essere a rischio di invalidità. Spesso si valuta una nuova variante depositabile + strumenti complementari (marchio, canali, contratti).'
         +'</p>';
     }
   }
   if(c.id==='public_inv'){
     if(answers.public_inv==='yes_pub'){
       h+='<p class="wz-sub" style="margin-top:12px;border:1px solid rgba(198,40,40,.28);background:rgba(198,40,40,.07);padding:10px;border-radius:10px">'
         +IC.alert+' <b>Stop brevetto:</b> In UE/Italia una divulgazione pubblica precedente al deposito <b>di norma distrugge la novità e impedisce il brevetto</b> (salvo eccezioni molto rare e ristrette). Il percorso verrà chiuso con un riepilogo operativo.'
         +'</p>';
     }
   }
