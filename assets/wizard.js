...
const STEPS = [
 ...
 /* ===== DIVULGAZIONE DESIGN (come prima) ===== */
 ...
 /* ===== DIVULGAZIONE BREVETTO (NUOVO: stop pulito) ===== */
 {id:'public_patent',question:'Hai già divulgato/pubblicato l\'invenzione (o dettagli tecnici) prima del deposito?',subtitle:'Per divulgazione intendiamo: post, pitch pubblici, demo, fiera, vendita/offerta, video, paper, o condivisione senza NDA. In UE/Italia una divulgazione pubblica precedente al deposito di norma distrugge la novità e impedisce il brevetto (salvo eccezioni molto rare e ristrette).',type:'single',
  showIf:function(a){return a.what==='invention';},
  options:[
   {value:'no',label:'No, è ancora riservata',desc:'Non ho divulgato pubblicamente dettagli tecnici.'},
   {value:'yes_pub',label:'Sì, ho già divulgato/pubblicato',desc:'È già stata resa pubblica (anche parzialmente) senza riservatezza.'},
   {value:'unsure_pub',label:'Non sono sicuro',desc:'Devo ricostruire cosa è stato condiviso e quando.'}
  ]},
...
const LABELS={
  ...
  goal:'Obiettivo',
  // FIX: allineato all'opzione del wizard (prima: "Prevenire copie/imitazioni")
  protect:'Proteggere / registrare',
...
\u2022 conserva prove di date e versioni (file, email, repository, screenshots).`},
 ...
 public_patent:{title:'Brevetto e divulgazione: perché è delicato',
 content:`Per i <b>brevetti</b> la regola pratica è semplice: <b>prima si deposita, poi si pubblica</b>.
In UE/Italia una divulgazione pubblica precedente al deposito <b>di norma distrugge la novità</b> e impedisce il brevetto (salvo eccezioni molto rare e ristrette).

Se hai già divulgato:
\u2022 non pubblicare altro
\u2022 ricostruisci <b>cosa</b> hai divulgato e <b>quando</b> (post, slide, demo, offerte, fiere, video)
\u2022 valuta strategie alternative (contratti, know\u2011how residuo, design, brand, ecc.).`},
...
function getRec(a){
  ...
  if(isActionPath){
    ...
    if(w==='invention'){
      // FIX: evita ambiguità "UE" vs "brevetto europeo (EPO/EPC)"
      var where3 = terr.indexOf('eu')!==-1
        ? 'tramite <b>brevetto europeo (EPO)</b> nei Paesi che ti interessano'
        : (terr.indexOf('italy')!==-1 && terr.indexOf('international')===-1
            ? 'con un primo deposito in Italia'
            : 'con una strategia multi\u2011Paese');

      if(pubPat==='yes_pub'){
        rec.title = "Brevetto: attenzione alla divulgazione";
        rec.text = "Hai indicato che l'invenzione (o dettagli tecnici) è stata <b>divulgata/pubblicata</b> prima del deposito. In UE/Italia una divulgazione pubblica precedente al deposito <b>di norma distrugge la novità e impedisce il brevetto</b> (salvo eccezioni molto rare e ristrette).";
        rec.next = [
          "Non pubblicare altro: evita ulteriori dettagli tecnici online o in presentazioni.",
          "Ricostruisci <b>cosa</b> è stato divulgato e <b>quando</b> (post, slide, video, demo, offerte, fiere) e conserva prove ordinate.",
          "Valuta alternative/complementi: <b>design</b> (se applicabile), <b>copyright</b> su documentazione/descrizioni/codice, <b>marchio</b> su nome/prodotto, e <b>contratti</b>/licenze per lo sfruttamento.",
          "Se vuoi una valutazione concreta delle eccezioni (rare), serve un check professionale mirato sul materiale divulgato."
        ];
      } else {
...
/* ===== pick: aggiunge "stop guidato" per brevetto ===== */
function pick(id,val){
  answers[id]=val;

  // STOP GUIDATO: se brevetto già divulgato, porta direttamente al riepilogo
  if(id==='public_patent' && val==='yes_pub'){
    step = totalSteps()-1;
    render();
    return;
  }
...
/* ===== MINI-LEZIONE (identica alla tua, solo publicState per invention usa public_patent) ===== */
function getLearn(a){
  var sit=a.situation||'';
  var what=a.what||'';
  var terr=Array.isArray(a.territory)?a.territory:[];
  var isOnline = terr.indexOf('online_global')!==-1 || what==='domain' || what==='digital_assets' || what==='software_ai';
  var publicState=(what==='invention' ? (a.public_patent||'') : (a.public||''));
  var pills=[];
...
function sendForm(){
  ...
  fetch("https://ip-wizard-api.vimana-grioni.workers.dev/send", {
    method: "POST",
  ...
  .finally(function(){
    if(submitBtn){
      ...
      submitBtn.innerHTML = IC.send + ' Invia richiesta';
    }
  });
}
...
function renderCore(){
 ...
 if(c.type==='single'||c.type==='multi'){
   ...
   /* ===== WARNING INLINE (design / brevetto / marchio / segreto) ===== */
   if(c.id==='what' && answers.what==='secret'){
     h+='<div class="wz-note" style="margin-top:12px">'+IC.alert+' <b>Nota sul segreto:</b> la tutela come segreto funziona solo se l\'informazione resta riservata (accessi, NDA, policy). Se è già pubblica, non è più un segreto.</div>';
   }

   if(c.id==='tm_use' && answers.tm_use){
     if(answers.tm_use==='use_yes'){
       h+='<div class="wz-note" style="margin-top:12px">'+IC.alert+' <b>Nota marchio:</b> se lo stai già usando, raccogli prove d\'uso (URL/screenshot datati, ADV, packaging, fatture) e valuta di depositare presto per rendere la tutela più difendibile.</div>';
     } else if(answers.tm_use==='use_no'){
       h+='<div class="wz-note" style="margin-top:12px">'+IC.alert+' <b>Nota marchio:</b> se non è ancora pubblico, spesso conviene pianificare il deposito prima del lancio (riduce il rischio di dover cambiare nome dopo aver investito).</div>';
     }
   }

   if(c.id==='public' && answers.public && answers.public!=='no' && answers.public!=='unsure_pub'){
     if(answers.public==='less_12'){
       h+='<div class="wz-note" style="margin-top:12px">'+IC.alert+' <b>Attenzione:</b> hai indicato una divulgazione recente. Per il design UE può esistere un margine (periodo di grazia), ma è limitato: muoviti presto e conserva prove ordinate (date/URL/versioni).</div>';
     } else if(answers.public==='more_12'){
       h+='<div class="wz-note" style="margin-top:12px">'+IC.alert+' <b>Attenzione:</b> hai indicato una divulgazione da oltre 12 mesi. Una registrazione di design UE può essere a rischio di invalidità: spesso conviene valutare nuove varianti depositabili e rafforzare strumenti complementari (brand, canali, contratti).</div>';
     }
   }

   if(c.id==='public_patent' && answers.public_patent==='yes_pub'){
     h+='<div class="wz-note" style="margin-top:12px">'+IC.alert+' <b>Stop (brevetto):</b> in UE/Italia una divulgazione pubblica precedente al deposito <b>di norma distrugge la novità e impedisce il brevetto</b> (salvo eccezioni molto rare e ristrette). Ti portiamo al riepilogo con i prossimi passi possibili.</div>';
   }
   ...
   var nextLabel = (c.id==='public_patent' && answers.public_patent==='yes_pub') ? 'Vai al riepilogo ' : 'Avanti ';
   fi.innerHTML='<button class="btn-ghost" onclick="goBack()">'+IC.arrowL+' Indietro</button>'
     +'<button class="btn btn-primary" onclick="goNext()" '+(canNext()?'':'disabled')+'>'+nextLabel+IC.arrowR+'</button>';
   return;
 }
